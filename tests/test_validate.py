        ...
        (
            ValidationReport(
                ...
                results=[
                    ValidationResult(
                        severity="violation",
                        message="Less than 1 values on <https://linked.data.gov.au/def/placenametype>->skos:hasTopConcept",
                    ),
                    ValidationResult(
                        severity="violation",
                        message="Less than 1 values on <https://linked.data.gov.au/def/placenametype>->skos:definition",
                    ),
                    ValidationResult(
                        severity="violation",
                        message="Requirement 2.1.5 Each vocabulary _MUST_ have one and only one modified date indicated using the dcterms:modified predicate that must be either an `xsd:date`, `xsd:dateTime or `xsd:dateTimeStamp` literal value",
                    ),
                    ValidationResult(
                        severity="violation",
                        message="Requirement 2.1.5 Each vocabulary _MUST_ have one and only one created date indicated using the dcterms:created predicate that must be either an `xsd:date`, `xsd:dateTime` or `xsd:dateTimeStamp` literal value",
                    ),
                ],
                results_text="""Validation Report
Conforms: False
...
def test_validate(report: ValidationReport, data: str, shacl_shapes: str):
    validation_report = validate(data, shacl_shapes)

    assert report.conforms == validation_report.conforms

    # We only partially check this because the validation results_text ordering is non-deterministic.
    assert report.results_text in validation_report.results_text

    for result in validation_report.results:
        assert result in report.results
